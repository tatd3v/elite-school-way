export default function Categories() {
  const categories = [
    ...
    { name: 'Femme Queen', icon: 'diamond', color: 'primary', description: 'Esencia femenina, poder trans. Realeza pura en el escenario.' },
  ...
  return (
    <section className="py-section-gap-desktop bg-surface-container-high" id="categories">
      <div className="max-w-7xl mx-auto px-margin-mobile md:px-margin-desktop">
        <div className="mb-16">
          <h2 className="font-headline text-headline-lg text-primary uppercase text-left border-l-8 border-secondary pl-6">
            CURRÍCULO DE COMPETENCIAS
          </h2>
          <p className="text-on-surface-variant mt-4 pl-8 max-w-2xl">
            Doce disciplinas para demostrar el linaje y el talento. Solo los más aptos alcanzarán la excelencia académica en la pasarela.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {categories.map((category) => (
            <div 
              key={category.name}
              className="bg-surface-container-lowest border border-outline-variant p-8 flex flex-col items-center text-center card-hover"
            >
